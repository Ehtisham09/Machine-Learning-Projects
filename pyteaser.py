...
stopWordsList=["کے","میں","کی","سے","اور","کا","نے","کو","ہے","پر","کہ","اس","بھی","ہے۔","کر","ایک","کیا","ہیں","۔","ان","یہ","ہیں۔","نہیں","جس","مطابق","ہو","گیا","کرنے","کہا","پاکستان","ہی","وہ","لیے","جبکہ","ساتھ","بعد","’","تو","ہوئے","‘","جو","فلم","اپنے","تک","ہے،","ہونے","،","والے","خان","رہے"]
# stopWords = set([
...
def Summarize(title, text):
    ...
    keys = keywords(text)
    titleWords = customtokenize(title)      #split_words(title)
    ...
    for item in summaries:
        print(item.encode('utf-8','ignore').decode('utf-8'))
    return summaries
...
def keywords(text):
    ...
    keywords=list()
    tokens=list()
    text=customtokenize(text)
    # allWordDist = nltk.FreqDist(text)
    # print('most common 50 words and frequencies')
    # for word, frequency in allWordDist.most_common(10):
    #     keywords.append(word)
    #print(repr(stopWordsList[0]))
    #print(repr(text[0]))
    for tok in text:
        tokens.append(tok.encode('utf-8','ignore'))
    #print(repr(tokens[0]))
    numWords = len(tokens)  # of words before removing blacklist words
    freq = Counter(x for x in tokens if x not in stopWordsList)
...
def title_score(title, sentence):
    title = [x for x in title if x not in stopWordsList]
    count = 0.0
    for word in sentence:
        if (word not in stopWordsList and word in title):
            count += 1.0
